...
pacman::p_load(tidyverse, cowplot, vegan, FactoMineR, readxl, ggpubr, MASS, RColorBrewer,
               readxl, growthrates, growthcurver, patchwork, betareg, lmtest)
```
...
And plot out to confirm data are ok:
```{r}
...
p_nd_diam_slope_lmlines_log<-nd_diam_slope %>%
  ...
  scale_colour_viridis_d(begin=0.4, end=0.75, option="inferno")+
  #scale_color_manual(values=c("red2", "darkblue")) +
  labs(title="", x="Time (h)", y="Expansion (cm/h)", color="Morph") +
  theme(text = element_text(size = xTextSize),
        axis.text.x = element_text(angle=90)) +
  facet_grid(vars(Peptone), vars(Community), scales="free_x")
...
ggsave("Fig4_NutrientMotility.tiff", width=240, height=180, units="mm")
# one column 80 mm width
```

Statistics. For simplicity we transform the data so the fit is linear:
```{r}
nd_diam<-nd_diam %>%
