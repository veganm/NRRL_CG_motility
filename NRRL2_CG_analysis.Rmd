...
Re-parse species names using community IDs
```{r}
# declare sets containing each bacterial species
hasRE15D<-c("A", "B", "C", "D")
hasRE15M<-c("E", "F", "G", "H")
hasRE16<-c("I", "J", "K", "L")
hasCG<-c("A", "B", "E", "F", "I", "J")
...
# pivot and assign
temp<-NRRLMulti2 %>%
  dplyr::select(!(varMO:varCX)) %>%
  pivot_longer(cols=AA:SX, names_to="Species", values_to="CFU")

temp$Species[temp$Species=="RE" & temp$Set %in% hasRE15D]<-"RE15D"
temp$Species[temp$Species=="RE" & temp$Set %in% hasRE15M]<-"RE15M"
temp$Species[temp$Species=="RE" & temp$Set %in% hasRE16]<-"RE16"
temp$Species[temp$Species=="CX"]<-"CI"
temp$Species[temp$Species=="CI" & temp$Set %in% hasCG]<-"CG"
temp$Species[temp$Species=="SX"]<-"SS"
temp$Species[temp$Species=="SS" & temp$Set %in% hasST]<-"ST"
#temp
```

Create stacked bar plots of relative abundance for all species:
```{r}
#   scale_fill_manual(values=c('red4', 'orangered', 'gold', 'yellowgreen', 'darkgreen', 'blue', 'purple')) # old scale 

pMultiF<-temp %>%
  filter(Pass=="1" | Pass=="5" | Pass=="10") %>% # we don't need to see ALL of these surely
  ggplot(aes(x=ID, y=CFU, fill=factor(Species))) +
  geom_bar(position="fill", stat="identity") +
  scale_fill_manual(values=c('red4',       #AA
                               'orangered',   #BS, 503 
                               'darkorange2',    # CG, 92, #EE7600
                               #'gold',        # CX, 142, #FFD700
                               'goldenrod2',   # CI, 149, #EEB422
                               #'yellowgreen',  # MO, 657 #9ACD32
                               #'darkgreen',    # OA, 85, #006400
                               'darkgreen',    # MO, 85, #006400
                               'darkolivegreen3',        # OA, 88, #A2CD5A
                               #'blue',        # RE, 26, #0000FF
                               'turquoise2',   # RE15D, 637, #00E5EE
                               'deepskyblue',  # RE15M, 121, #00BFFF
                               'blue',         # RE16, 26, #0000FF
                               #'purple'       # SX, 547, #A020F0
                               'purple',        # SS
                               'mediumorchid1' # ST, 463, #E066FF
                               )) + 
  theme(
    axis.text=element_text(size=xTextSize+1), 
...
Re-label species full IDs:
```{r}
temp2<-MultiNoWorm %>%
  dplyr::select(!Total) %>%
  pivot_longer(cols=AA:SX, names_to="Species", values_to="CFU")

temp2$Species[temp2$Species=="RE" & temp2$Set %in% hasRE15D]<-"RE15D"
temp2$Species[temp2$Species=="RE" & temp2$Set %in% hasRE15M]<-"RE15M"
temp2$Species[temp2$Species=="RE" & temp2$Set %in% hasRE16]<-"RE16"
temp2$Species[temp2$Species=="CX"]<-"CI"
temp2$Species[temp2$Species=="CI" & temp2$Set %in% hasCG]<-"CG"
temp2$Species[temp2$Species=="SX"]<-"SS"
temp2$Species[temp2$Species=="SS" & temp2$Set %in% hasST]<-"ST"
temp2
```
...
pMultiPlotsTop_Merge_BC<-plot_grid(pMultiF_NW, pMultiF,
                          rel_widths=c(1, 1.3), labels=c("C", "D"))
pMultiPlotsTop_Merge<-plot_grid(NULL, pPCAMNW_Merge, pMultiPlotsTop_Merge_BC,
                                ncol=1, rel_heights=c(0.6, 1, 1), labels=c("A", "B", ""))
...
```{r}
pMultiPlotsBottom<-plot_grid(pVariantEmergenceHeat, NULL, NULL, ncol=1, 
                             rel_heights=c(1.8,1,0.1),
                             labels=c("E", "F",""))
plot_grid(pMultiPlotsTop_Merge, pMultiPlotsBottom, ncol=2,
          rel_widths = c(1, 0.9))
#ggsave("pFig1_Multispecies_AllPassSummary_Merge_Heat_ivory1_grey80.png", width=10, height=12, units="in", dpi=400)
ggsave("pFig1_Multispecies_AllPassSummary_Merge_Heat_grey99_blackfill.tiff", width=338, height=200, units="mm")
...
#  labs(y="Inverse Simpson", x="Pass", title="C. gleum")+
    labs(y="Inverse Simpson", x="Pass", title="")+
  #facet_wrap(~Variant, ncol=1)+
  theme(strip.text=element_text(size=xTextSize, face="italic"))
...
#          ncol=2, align="h")
plot_grid(pVariantEmergenceRichness_CG,
          pVariantEmergenceH_CG,
          pVariantEmergenceIS_CG,
          ncol=1, align="v")
ggsave("FigS3_EmergenceMorphVariants.png", height=6, width=4, units="in", dpi=300)
```
...
Reorder pass as factor:
```{r}
motility_agarconc$Pass<-factor(motility_agarconc$Pass, levels=c("0", "6", "10"))
```
